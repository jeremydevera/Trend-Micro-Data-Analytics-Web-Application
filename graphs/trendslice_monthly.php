...
<?php 
  ...
  $query2 = "select 'Not Supported' as trendx, COUNT(*) as counter from jeremy_table_trend WHERE trendx LIKE '%Not Supported%' and 
  (date_sourced BETWEEN '$date_from' AND '$date_to')";
  $NOTSUPORTED = mysqli_query($conn, $query2);
  
  $query3 = "select 'Escalate' as trendx, COUNT(*) as counter from jeremy_table_trend WHERE trendx LIKE '%Escalate%' and 
  (date_sourced BETWEEN '$date_from' AND '$date_to')";
  $ESCALATE = mysqli_query($conn, $query3);

  $query4 = "select 'Detected' as trendx, COUNT(*) as counter from jeremy_table_trend WHERE trendx NOT LIKE '%Not Supported%' AND trendx NOT LIKE '%Undet%' AND trendx NOT LIKE '%Escalate%' and 
  (date_sourced BETWEEN '$date_from' AND '$date_to')";
  $DETECTED = mysqli_query($conn, $query4);

  //SHOW DISTINCT FROM TRENDX
  $query5 = "SELECT DISTINCT trendx, COUNT(*) as counter FROM jeremy_table_trend WHERE trendx LIKE '%Undet%' and 
  (date_sourced BETWEEN '$date_from' AND '$date_to') group by trendx";
  $sub_undet= mysqli_query($conn, $query5);
 

  $query6 = "SELECT DISTINCT trendx, COUNT(*) as counter FROM jeremy_table_trend WHERE trendx LIKE '%Not Supported%' and 
  (date_sourced BETWEEN '$date_from' AND '$date_to') group by trendx";
  $sub_notsupported= mysqli_query($conn, $query6);


  $query7 = "SELECT DISTINCT trendx, COUNT(*) as counter FROM jeremy_table_trend WHERE trendx LIKE '%Escalate%' and 
  (date_sourced BETWEEN '$date_from' AND '$date_to') group by trendx";
  $sub_escalate = mysqli_query($conn, $query7);

  $query8 = "SELECT DISTINCT trendx, COUNT(*) as counter FROM jeremy_table_trend WHERE trendx NOT LIKE '%Not Supported%' AND trendx NOT LIKE '%Undet%' AND trendx NOT LIKE '%Escalate%' and 
  (date_sourced BETWEEN '$date_from' AND '$date_to') group by trendx";
